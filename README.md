...
spoken is not configurable per server (see [HTTP.md]).  The confiugration section allows the following configuration
options:
...
- `enable-http2`: boolean to enabls the usage of http2 on the request.  There seems to be some incompatibility with the
